...
export default function CommitteeLogin() {
  ...
  useEffect(() => {
    if (isAuthenticated && user?.role === 'committee') {
      navigate(ROUTES.COMMITTEE, { replace: true })
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      login(formData.email, formData.password, 'committee')
      showToast('Committee member logged in successfully.', 'success')
      navigate(ROUTES.COMMITTEE)
    } catch (error) {
      showToast('Committee login failed. Please try again.', 'error')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="relative hidden min-h-[420px] lg:block">
            ...
            <div className="relative z-10 flex h-full flex-col items-center justify-center px-10 text-center text-white">
              <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-full border border-white/30 bg-white/20 backdrop-blur">
                <i className="material-icons-round text-4xl">groups</i>
              </div>
              <h2 className="font-primary text-3xl font-bold">E-Masjid Committee Portal</h2>
              <p className="mt-4 max-w-md leading-relaxed text-white/90">
                Review and approve fund requests, manage Zakat disbursements, and oversee financial transparency for your mosque.
              </p>
          ...
          <div className="px-6 py-10 sm:px-10 sm:py-12">
            <div className="mx-auto max-w-md">
              ...
              <div className="mb-7 mt-6">
                <h1 className="font-primary text-4xl font-bold text-gray-900">Committee Login</h1>
                <p className="mt-2 text-gray-600">Authorized committee member access only.</p>
              </div>
              ...
              <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                <div className="flex items-start gap-2">
                  ...
                  <p>
                    This portal is exclusively for verified committee members to manage community funds.
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="form-label" htmlFor="committee-email">
                    Committee Email <span className="text-red-500">*</span>
                  </label>
                ...
                <div>
                  <label className="form-label" htmlFor="committee-password">
                ...
                >
                  ...
                  {loading ? 'Signing in...' : 'Sign In to Committee Panel'}
                </button>
              </form>
            </div>
          </div>
